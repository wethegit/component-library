...
The spacing utility allows you to easily establish a consistent *vertical* flow for the child elements of a given container. This is a useful alternative to adding `spacing.margin.top[2]` to every child element.
...
This uses the CSS selector `> * + *` behind the scenes, adding `margin-top` to every *subsequent* child of an element (ommitting the first child).
