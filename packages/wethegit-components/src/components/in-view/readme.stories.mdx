...
As shown in the example above, you can use animation presets by importing the `animation` object from the [animation utilities](). Behind the scenes, these preset animation properties just point to CSS modules, so you're free to pass in your own CSS modules to the `animation` prop instead! Animation modules should look something like this, below.
